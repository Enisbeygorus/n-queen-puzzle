...
class ChessBoardContainer extends Component {
    state = {
      ...
      componentDidUpdate() {
        ...
        if(this.state.queenPlacement.length === 16 && this.state.queenClick !== false) {
            this.setState({queenClick: false})
        }

        if(this.state.queenPlacement.length < 16 && this.state.queenClick !== true){
      ...
      queenClickHandler = (row, column) => {
        ...
        for(let i = 0; i < this.state.queenPlacement.length; i+=2){
            if(this.state.queenPlacement[i] === row && this.state.queenPlacement[i+1] === column){
                let val = this.state.board[row][column];
                if (val === 1) {
                  val = 0;
                } else {
